...
def parse_natural_date(
    text: str, reference_date: Optional[date] = None, languages: List[str] = ["en"]
) -> Optional[str]:
...
def is_within_budget(
    total_cost: float, budget: float, buffer_pct: float = 0.95
) -> bool:
...
def resolve_airport_code(location: str) -> Optional[str]:
    ...
    # Already looks like IATA code (3 uppercase letters)
    if len(stripped) == 3 and stripped.isupper() and stripped.isalpha():
        return stripped

    # City name lookup (normalized)
    return NEPAL_AIRPORTS.get(stripped.lower())
